		...
		config = function()
			...
			local pickers = {
				...
				live_grep = function(opts)
					...
					global_state.set_global_key('live_grep:hidden', hidden) -- better solutions?
					opts.hidden = nil
					if hidden then
						opts.additional_args = vim.tbl_flatten({ opts.additional_args or {}, { "--hidden" } })
					end
			...
			local my_actions = require("telescope.actions.mt").transform_mod({
				live_grep_toggle_hidden = function(prompt_bufnr)
					local hidden = global_state.get_global_key('live_grep:hidden') or false
					pickers.live_grep({ hidden = not hidden })
				end,
			...
			telescope.setup({
				...
				pickers = {
					find_files = {
						mappings = {
							n = {
								[";g"] = actions.close + my_actions.live_grep, -- TODO: actions to toggle options
			...
			vim.keymap.set('n', ';g', pickers.live_grep, getkopts({ desc = "grep" }))
