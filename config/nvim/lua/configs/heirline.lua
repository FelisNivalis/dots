    ...
    local FileName = {
        {
            ...
            provider = function(self)
                for _, func in ipairs({
                    ...
                    local s = func(self)
                    if conditions.width_percent_below(#s, 0.1) and s ~= '' then
                        return s .. '  '
                    end
                    return ''
                end
        ...
        },
        {
            provider = function(self)
                ...
                for _, func in ipairs({
                    ...
                    function (self) return vim.fn.pathshorten(vim.fn.fnamemodify(self.filename, ":.:h"), 2) ..
                        '/' .. vim.fn.fnamemodify(self.filename, ":t") end,
                    function (self) return vim.fn.pathshorten(vim.fn.fnamemodify(self.filename, ":.:h"), 1) ..
                        '/' .. vim.fn.fnamemodify(self.filename, ":t") end,
                    function (self) return vim.fn.fnamemodify(self.filename, ":t") end,
                }) do
                    s = func(self)
                    if conditions.width_percent_below(#s, 0.1) then
                        return s or "[No Name]"
                    end
                end
                        ...
                        if difftime <= 50 then
                            ...
                            if difftime < 100 then
                                ...
                                if dt_table.year > 1970 then
                                    unit = 'y'
                                    difftime = dt_table.year - 1970 + (dt_table.month or 0) / 12 + (dt_table.day or 0) / 365
