{ lib, pkgs, config, ... }: let
	...
	packages = {
		...
		nvim-install-env = pkgs.writeShellScriptBin "nvim-install-env" ''
			...
			if [ -n "$update" ] && [ "$(${pyenv} latest -k 3)" != "$(${pyenv} global)" ] && [ -d "$PYENV_ROOT/versions/neovim" ]; then
			...
			if [ -n "$update" ] && [ "$(${rbenv} install --list | grep -v - | tail -n 1 | sed 's/^[ \t]*//;s/[ \t]*$//')" != "$(${rbenv} global)" ] && [ -d "$RBENV_ROOT/versions/$(${rbenv} global 2>/dev/null)/gemsets/neovim" ]; then
				${rbenv} gemset delete $(${rbenv} global 2>/dev/null) neovim
			fi
		...
		mise-install-all = pkgs.writeShellScriptBin "mise-install-all" ''
			...
			if ${mise} ls --global >/dev/null 2>&1 && \
				[ -z "$(${mise} ls --global --missing --no-header)" ] && \
				[ -z "$(${mise} ls --global | grep -E 'missing|outdated')" ]; then
				...
				if [ -z "$(${mise} ls --global --missing --no-header)" ]; then
					${mise} --cd $HOME install --verbose --yes
				fi
				if [ -z "$(${mise} upgrade --dry-run)" ]; then
		...
		start-jupyter = pkgs.writeShellScriptBin "start-jupyter" ''
			...
			(jupyter-kernelspec list | grep bash >/dev/null) || \
			  (echo "Installing bash kernel..." && python -m zsh_jupyter_kernel.install --user)
			...
			(jupyter-kernelspec list | grep sos >/dev/null) || \
				(echo "Installing Script of Script..." && python -m sos_notebook.install --prefix $XDG_DATA_HOME/..)

			mamba_env_name=jupyter-kernels
			if ! (${micromamba} list --name $mamba_env_name | grep xeus-lua); then
			...
			export JUPYTER_PATH=''${JUPYTER_PATH:+$JUPYTER_PATH:}"$MAMBA_ROOT_PREFIX/envs/jupyter-kernels/share/jupyter:$XDG_DATA_HOME/jupyter"
			echo "Run jupyter with kernels:"
			jupyter-kernelspec list

			if [ -z "$XDG_DOCUMENTS_DIR" ]; then
			  echo "'XDG_DOCUMENTS_DIR' not set, starting jupyter at '$HOME/jupyter'..."
			fi
			jupyter_data_path="''${XDG_DOCUMENTS_DIR:-$HOME}/jupyter"
